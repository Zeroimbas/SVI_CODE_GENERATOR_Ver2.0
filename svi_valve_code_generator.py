...
GGC_TRIM = ["1","2","3","4","5","5a","6","7","8","8a","9","10","11","11a","12","12a","13","14","14a","15","16","17","17/4PH","18","304","316","321","347","ALLOY 20","ALLOY 625","C276","F6","MONEL®","St  Gr6","NONE"]
...
GGC_STEM_MATERIAL = ["410(1Cr13)","F304L","A105N/ENP","17-4PH","AISI 4140/ENP","F316","LF2/ENP","F51","1Cr13/ENP","F53","H.C276","F316/ENP","A105N-Cr","F51/ENP","INCOLOY 825","1Cr13(3)","F321","NITRONIC","17-7PH","F44(U31254)","INCONEL600","FXM-19+ENP","Inconel 625","DUCTILE IRON","F55","F317L","F60","F347"]
...
BV_ORING_MATERIAL = ["BUNA-N","ELAST-0-LION 101","EPDM","FFKM","FR 58/90","HNBR","HNBR-70","HNBR-82","HNBR-ED (Elast-O-Lion 985)","Lipseal","NBR","PCTFE","PTFE JACKETED VITON","TBD","VITON A","VITON AED","VITON B","VITON F","VITON GF","VITON GLT","VMQ","NONE"]
...
def popupmissing(Errorlist, Valve_type):
        popup = tk.Tk()
        ...
        Result = "THE FOLLOWING OPTIONS CANNOT BE EMPTY FOR %s: \n" %Valve_type + "\n".join(Errorlist)
...
def popupwarning(warning):
    ...
    label = tk.Label(popup, text= warning)
    label.grid()
    B1 = ttk.Button(popup, text = "Ok", command = leavemini)
    B1.grid()
    popup.tkraise()
...
class PageOne(tk.Frame):
    ...
    def generate_code(self):
        ...
        Disc_Material = self.label_8_Combo.get()
            #if Disc_Material == "":
            #    raise ValueError ("DISC MATERIAL EMPTY")
        ...
        Stem_Material = self.label_9_Combo.get()
            #if Stem_Material == "":
            #    raise ValueError ("STEM MATERIAL EMPTY")
        Stem_Material_Code = GGC_STEM_MATERIAL_CODE.get(Stem_Material)

        #Stem_Material_Code = GGC_STEM_MATERIAL_CODE.get(Stem_Material)
        
        
        Seat_Material = self.label_10_Combo.get()
        Seat_Material_Code = GGC_SEAT_MATERIAL_CODE.get(Seat_Material)
        
        Packing_Material = self.label_11_Combo.get()
        Packing_Material_Code = GGC_PACKING_MATERIAL_CODE.get(Packing_Material)
        
        Oring_Material = self.label_12_Combo.get()
        Oring_Material_Code = GGC_ORING_MATERIAL_CODE.get(Oring_Material)
        
        Design_Standard = self.label_13_Combo.get()
        
        Operation_Type = self.label_14_Combo.get()
        Operation_Type_Code = GGC_OPERATION_TYPE_CODE.get(Operation_Type)
        
        Design_Feature = self.label_15_Combo.get()
        Design_Feature_Code = GGC_DESIGN_FEATURE_CODE.get(Design_Feature)
        
        Fire_Safe = self.label_16_Combo.get()
    
        Nace = self.label_17_Combo.get()
        
        Lock = self.label_18_Combo.get()

        #Could_be_None_List = [Oring_Material, Design_Standard, Operation_Type, Design_Feature, Fire_Safe, Nace, Lock]
        #Check_Necessary_List = {Body_Material:"BODY MATERIAL", Packing_Material:"PACKING MATERIAL", Design_Feature:"DESIGN FEATURE", Size:"SIZE", ANSI_Class:"ANSI CLASS"}
        
        Check_Necessary_List = [Body_Material, Disc_Material, Seat_Material, Size, ANSI_Class, End_Connection]
        Check_Necessary_List_Output = ["BODY MATERIAL", "DISC MATERIAL", "SEAT MATERIAL", "SIZE", "ANSI CLASS", "END CONNECTION"]
        GG_Necessary_List = [Body_Material, Size, ANSI_Class, Stem_Material, Operation_Type, End_Connection, Disc_Material, Seat_Material]
        GG_Necessary_List_Output = ["BODY MATERIAL", "SIZE", "ANSI CLASS", "STEM MATERIAL", "OPERATION TYPE", "END CONNECTION", "DISC MATERIAL", "SEAT MATERIAL"]
        if (Item == ""):
            popupwarning("ITEM CANNOT BE EMPTY!")
        
        elif ("CHECK" in Item):
            for i in [i for i,x in enumerate(Check_Necessary_List) if x == ""]:
                Errorlist.append(Check_Necessary_List_Output[i])
            if Errorlist:
                popupmissing(Errorlist, "CHECK VALVE") 
            else:
                Codelist_First.extend((Item_Code, Body_Material_Code, Size_Code, ANSI_Class_Code))
                Codelist_Second.extend((Disc_Material_Code, Seat_Material_Code, Packing_Material_Code, Port_Type_Code, Valve_Design_Type_Code, Design_Feature_Code))
                Code = "PART#:    " + "-".join(Codelist_First) + "/" + "".join(Codelist_Second)
                Special_Note = self.special_Note_TextBox.get("1.0","end-1c")
                Description = "Description: " + Item + ", " + Size + ", " + "CLASS " + ANSI_Class + "," + End_Connection + ", " + Port_Type + ", " + "BODY " + Body_Material + ", " + "DISC " + Disc_Material + ", " + "SEAT " + Seat_Material + ", " + "ORING " + Oring_Material + ", " + "PACKING " + Packing_Material + ", " + Fire_Safe + ", " + Nace + ", " + Lock + ". "
                
        elif ("CHECK" not in Item):
            for i in [i for i,x in enumerate(GG_Necessary_List) if x == ""]:
                    Errorlist.append(GG_Necessary_List_Output[i])
            if Errorlist:
                popupmissing(Errorlist, "GATE VALVE")
            else:
                Codelist_First.extend((Item_Code, Body_Material_Code, Size_Code, ANSI_Class_Code))
                Codelist_Second.extend((Disc_Material_Code, Stem_Material_Code, Seat_Material_Code, Packing_Material_Code, Port_Type_Code, Valve_Design_Type_Code, Design_Feature_Code))
                Code = "PART#:    " + "-".join(Codelist_First) + End_Connection_Code + Operation_Type_Code + "/" + "".join(Codelist_Second)
                Special_Note = self.special_Note_TextBox.get("1.0","end-1c")
                Description = "Description: " + Item + ", " + Size + ", " + "CLASS " + ANSI_Class + ", " + End_Connection + ", " + Port_Type + ", " + "BODY " + Body_Material + ", " + "DISC " + Disc_Material + ", " + "STEM " + Stem_Material + ", " + "SEAT " + Seat_Material + ", " + "PACKING " + Packing_Material + ", " + Operation_Type + ", " + Fire_Safe + ", " + Nace + ", " + Lock + ". "
                
        
        #print(re.findall(r'[,]\sNONE', Description))
        #re.sub(r'[,]\sNONE', r'', Description)        

        if (Codelist_First and Description):
            Description = re.sub(r'[,]\sNONE', '', Description)
        ...
        #Disc_Material_Code + Stem_Material_Code + Seat_Material_Code + Packing_Material_Code + Port_Type_Code + Valve_Design_Type_Code + Design_Feature_Code    
        #if (Errorlist and "CHECK" not in Item):
        #    	popupmissing(Errorlist)
        
        #if("CHECK" not in Item and Body_Material and Disc_Material and Stem_Material):
        #    Codelist_First.extend((Item_Code, Body_Material_Code, Size_Code, ANSI_Class_Code))
        #    Code = "PART#:    " + "-".join(Codelist_First)
        #    Result = Code + "\n" + Description
        #    popupmsg(Result)
        #else:
    ...
    def set_default(self):
        self.label_1_Combo.set('')
        self.label_2_Combo.set('')
        self.label_3_Combo.set('')
        self.label_4_Combo.set('')
        self.label_5_Combo.set('')
        self.label_6_Combo.set('')
        self.label_7_Combo.set('')
        self.label_8_Combo.set('')
        self.label_9_Combo.set('')
        self.label_10_Combo.set('')
        self.label_11_Combo.set('')
        self.label_12_Combo.set('')
        self.label_13_Combo.set('')
        self.label_14_Combo.set('')
        self.label_15_Combo.set('')
        self.label_16_Combo.set('')
        self.label_17_Combo.set('')
    ...
    def __init__(self, parent, controller):
        ...
        self.label_1_Combo = ttk.Combobox(self, textvariable = tk.StringVar(),value = GGC_ITEM, state = 'readonly')
        self.label_1_Combo.set("")
        self.label_1_Combo.grid(row = 2, column = 1, sticky = "we")
...
class PageTwo(tk.Frame):
    ...
    def generate_code(self):
        ...
        Ball_Material = self.label_8_Combo.get()
        ...
        Fire_Safe = self.label_15_Combo.get()
    
        Nace = self.label_16_Combo.get()
        
        Lock = self.label_17_Combo.get()
        
        BV_Necessary_List = [Valve_Design_Type, Body_Material, Size, ANSI_Class, Stem_Material, Operation_Type, End_Connection, Ball_Material, Seat_Material]
        BV_Necessary_List_Output = ["VALVE DESIGN TYPE", "BODY MATERIAL", "SIZE", "ANSI CLASS", "STEM MATERIAL", "OPERATION TYPE", "END CONNECTION", "BALL MATERIAL", "SEAT MATERIAL"]
        if (Item == ""):
            popupwarning("ITEM CANNOT BE EMPTY!")

        else:
            for i in [i for i,x in enumerate(BV_Necessary_List) if x == ""]:
                Errorlist.append(BV_Necessary_List_Output[i])
            if Errorlist:
                popupmissing(Errorlist, "BALL VALVE")
            else:
                Codelist_First.extend((Valve_Design_Type_Code, Body_Material_Code, Size_Code, ANSI_Class_Code))
                Codelist_Second.extend((Ball_Material_Code, Port_Type_Code, Seat_Material_Code))
                Code = "PART#:    " + "-".join(Codelist_First) + End_Connection_Code + Operation_Type_Code + "/" + "".join(Codelist_Second)
                Special_Note = self.special_Note_TextBox.get("1.0","end-1c")
                Description = "Description: " + Item + ", "  + Size + ", " + "CLASS " + ANSI_Class + ", " + Valve_Design_Type + "," + End_Connection + ", " + Port_Type + ", " + "BODY " + Body_Material + ", " + "BALL " + Ball_Material + ", " + "STEM " + Stem_Material + ", " + "SEAT " + Seat_Material + ", " + "PACKING " + Packing_Material + ", " + "ORING " + Oring_Material + ", " + Design_Standard + ", " + Operation_Type + ", " + Fire_Safe + ", " + Nace + ", " + Lock + ". "
...
class PageThree(tk.Frame):
    ...
    def generate_code(self):
        ...
        End_Connection = self.label_4_Combo.get()
        End_Connection_Code = GGC_END_CONNECTION_CODE.get(End_Connection)
        
        Port_Type = self.label_5_Combo.get()
        #Port_Type_Code = GGC_PORT_TYPE_CODE.get(Port_Type)
        
        Body_Material = self.label_6_Combo.get()
        Body_Material_Code = GGC_BODY_MATERIAL_CODE.get(Body_Material)
        
        #Disc_Material = GGC_DISC_MATERIAL[self.label_8_Combo.current()]
        #Disc_Material_Code = GGC_DISC_MATERIAL_CODE.get(Disc_Material)
        
        Disc_Trim = self.label_7_Combo.get()
        ...
        Packing_Material = self.label_8_Combo.get()
        #Packing_Material_Code = GGC_PACKING_MATERIAL_CODE.get(Packing_Material)
        ...
        Operation_Type = self.label_9_Combo.get()
        Operation_Type_Code = GGC_OPERATION_TYPE_CODE.get(Operation_Type)
        
        Design_Standard = self.label_10_Combo.get()
        #Design_Feature = GGC_DESIGN_FEATURE[self.label_16_Combo.current()]
        #Design_Feature_Code = GGC_DESIGN_FEATURE_CODE.get(Design_Feature)
        
        Fire_Safe = self.label_11_Combo.get()
        #Fire_Safe_Code = GGC_FIRE_SAFE_CODE.get(Fire_Safe)
    
        Nace = self.label_12_Combo.get()
        #Nace_Code = GGC_NACE_CODE.get(Nace)
        
        Lock = self.label_13_Combo.get()
        #Lock_Code = GGC_LOCK_CODE.get(Lock)
        
        GG_Trim_Necessary_List = [Body_Material, Size, ANSI_Class, End_Connection, Operation_Type, Disc_Trim]
        GG_Trim_Necessary_List_Output = ["BODY MATERIAL", "SIZE", "ANSI CLASS", "END CONNECTION", "OPERATION TYPE", "DISC TRIM"]
        CHECK_Trim_Necessary_List = [Body_Material, Size, ANSI_Class, End_Connection, Disc_Trim]
        CHECK_Trim_Necessary_List_Output = ["BODY MATERIAL", "ANSI CLASS", "END CONNECTION", "DISC TRIM"]
        if (Item == ""):
            popupwarning("ITEM CANNOT BE EMPTY!")
        
        elif ("CHECK" in Item):
            for i in [i for i,x in enumerate(CHECK_Trim_Necessary_List) if x == ""]:
                Errorlist.append(CHECK_Trim_Necessary_List_Output[i])
            if Errorlist:
                popupmissing(Errorlist, "CHECK VALVE") 
            else:
                Codelist_First.extend((Item_Code, Body_Material_Code, Size_Code, ANSI_Class_Code))
                Codelist_Second.extend((Disc_Trim_Code))
                Code = "PART#:    " + "-".join(Codelist_First) + "/" + "".join(Codelist_Second)
                Special_Note = self.special_Note_TextBox.get("1.0","end-1c")
                Description = "Description: " + Item + ", " + Size + ", " + "ANSI CLASS " + ANSI_Class + "," + End_Connection + ", " + Port_Type + ", " + "BODY " + Body_Material + ", " + "TRIM# " + Disc_Trim + ", " + Design_Standard + ", " + "PACKING " + Packing_Material +  ", " + Fire_Safe + ", " + Nace + ", " + Lock + ". "
                
        elif ("CHECK" not in Item):
            for i in [i for i,x in enumerate(GG_Trim_Necessary_List) if x == ""]:
                    Errorlist.append(GG_Trim_Necessary_List_Output[i])
            if Errorlist:
                popupmissing(Errorlist, "GATE VALVE")
            else:
                Codelist_First.extend((Item_Code, Body_Material_Code, Size_Code, ANSI_Class_Code))
                ...
                Code = "PART#:    " + "-".join(Codelist_First) + End_Connection_Code + Operation_Type_Code + "/" + "".join(Codelist_Second)
                Special_Note = self.special_Note_TextBox.get("1.0","end-1c")
                Description = "Description: " + Item + ", " + Size + ", " + "ANSI CLASS " + ANSI_Class + ", " + End_Connection + ", " + Port_Type + ", " + "BODY " + Body_Material + ", " + "TRIM# " + Disc_Trim + ", " + Design_Standard + ", "  + Operation_Type + ", " + "PACKING " + Packing_Material +  ", " + Fire_Safe + ", " + Nace + ", " + Lock + ". "
